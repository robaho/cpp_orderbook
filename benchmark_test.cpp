...
void insertOrders(const bool withTrades,const int PRICE_LEVELS) {
    ...
    for(int i=0;i<N_ORDERS;i++) {
        ob.insertOrder(new (ob.allocateOrder()) TestOrder(i,5000.0 + 1 * (i%PRICE_LEVELS),10,Order::BUY));
    }
    for(int i=0;i<N_ORDERS;i++) {
        ob.insertOrder(new (ob.allocateOrder()) TestOrder(N_ORDERS+i,(withTrades ? 5000.0 : 10000.0) + 1 * (i%PRICE_LEVELS),10,Order::SELL));
    }
...
void cancelOrders(const int PRICE_LEVELS) {
    ...
    for(int i=0;i<N_ORDERS;i++) {
        auto order = new (ob.allocateOrder()) TestOrder(i,100.0 + 1 * (i%PRICE_LEVELS),10,Order::BUY);
